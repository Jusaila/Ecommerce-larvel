         ...
         <div class="col-md-12">
           <div class="card">
             <div class="card-header">
               <h3 class="card-title">Website Content List</h3>
             </div>
             <div class="card-header ">
                <a href="{{ route('website.create') }}"><button class="bg-success">Add New</button></a>
              </div>
             <!-- /.card-header -->
             <div class="card-body">
               <table class="table table-bordered">
                 ...
                 <tbody>
                    ...
                    <tr>
                        <td>{{ $website->id }}</td>
                        <td>{{ $website->content_id }}</td>
                        <td>{{ $website->heading }}</td>
                        <td>{{ $website->content }}</td>
                        <td><img style="width: 50px;height:50px;" src="{{ $website->image }}"></td>
                        ...
                        {{-- <td>
                            ...
                            <form method="POST" action="{{ route('website-delete') }}"  onsubmit="return confirm('Do you want delete?')" style="display:inline;">
                                @csrf
                                <input type="hidden" name="id" value="{{ $website->id }}">
                                <button class="btn btn-danger" type="submit">Delete</button>
                            </form>
                            <a href="{{ url('website/edit/'.$website->id) }}"><button class="btn btn-primary">Edit</button></a>
                        </td> --}}
                      </tr>
                    @endforeach
