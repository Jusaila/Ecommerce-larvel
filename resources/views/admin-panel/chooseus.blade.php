        ...
        @endif
         <div class="col-md-12">
           <div class="card">
             ...
             <div class="card-header ">
                <a href="{{ route('chooseus.create') }}"><button class="bg-success">Add New</button></a>
              </div>
             <!-- /.card-header -->
             <div class="card-body">
               <table class="table table-bordered">
                 ...
                 <tbody>
                    @foreach ($chooseuses as $index=>$chooseuse)
                    <tr>
                        <td>{{ $chooseuse->id }}</td>

                        <td><img style="width: 50px;height:50px;" src="{{ $chooseuse->icon }}"></td>

                        <td>{{ $chooseuse->sub_heading }}</td>
                        <td>{{ $chooseuse->sub_content }}</td>


                        <td>
                            @if ($chooseuse->status == "Active")
                        ...
                        <td>
                            ...
                            <form method="POST" action="{{ route('chooseus.delete') }}"  onsubmit="return confirm('Do you want delete?')" style="display:inline;">
                                @csrf
                                <input type="hidden" name="id" value="{{ $chooseuse->id }}">
  ...
  <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
    {{-- <script>
        $(document).ready(function(){
            var chooseuseList = @json($chooseuses);
            $('.view-details').on('click',function(){
                var index = $(this).data('index');
                var chooseuse = chooseuseList[index];
                $('#id').text(chooseuse.id);
                $('#heading').text(chooseuse.heading);
                $('#content').text(chooseuse.content);
                $('#image').attr('src',chooseuse.image);
                console.log(chooseuse);
            });
             });
    </script> --}}
