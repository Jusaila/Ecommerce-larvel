       ...
       <div class="row mb-2">
         ...
         <div class="col-sm-6">
           <ol class="breadcrumb float-sm-right">
             ...
             <li class="breadcrumb-item active">Add Excellent Materials</li>
           </ol>
         </div>
       </div>
     </div><!-- /.container-fluid -->
         ...
         <div class="col-md-12">
           <div class="card">
             <div class="card-header">
               <h3 class="card-title">Excellent Materials List</h3>
             </div>
             <div class="card-header ">
                <a href="{{ route('excellent.create') }}"><button class="bg-success">Add New</button></a>
              </div>
             <!-- /.card-header -->
             <div class="card-body">
               <table class="table table-bordered">
                 ...
                 <tbody>
                    @foreach ($excellents as $index=>$excellent)
                    <tr>
                        <td>{{ $excellent->id }}</td>
                        <td>{{ $excellent->heading }}</td>
                        <td>{{ $excellent->content }}</td>

                        <td>
                            @if ($excellent->status == "Active")
                        ...
                        <td>
                            ...
                            <form method="POST" action="{{route('banner-delete')}}"  onsubmit="return confirm('Do you want delete?')" style="display:inline;">
                                @csrf
                                <input type="hidden" name="id" value="{{$excellent->id}}">
  ...
  <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
    <script>
        $(document).ready(function(){
            var ExcellentList = @json($excellents);
            $('.view-details').on('click',function(){
                var index = $(this).data('index');
                var Excellent = ExcellentList[index];
                $('#id').text(Excellent.id);
                $('#heading').text(Excellent.heading);
                $('#content').text(Excellent.content);
                $('#image').attr('src',Excellent.image);
